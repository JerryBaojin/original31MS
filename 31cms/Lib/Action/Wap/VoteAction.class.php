 ...
 private  $tourvote;
   ...
   /* public function __construct()
    ...
    public function _initialize()
    {
        ...
        $this->tourvote=$ars[0]['tourvote'];

        parent::_initialize();
        $IIIIIllII1ll = 6;
        C('site_url', 'http://' . $_SERVER['HTTP_HOST']);
    }
//    protected $user_is_gz = 1;
    public function index()
    {
        ...
        if (isWeixin()){
            ...
            if (!isset($_SESSION['act'])) {
                header("location:$url/userinfo/index.php?id=$IIIIIllI1l1l&token=$IIIIIIIIlIlI");
                return false;
            } 
            }
        else{//假冒游客
                 function guid(){
                        ...
                        if (!$_COOKIE['wxd_openid']) {
                                ...
                                $ope= guid();
                                $data['openid']=$ope;
                                setcookie('wxd_openid', $ope, time() + 31536000);
                                $re=M('fusers')->data($data)->add();
                        }else{
                             $a=$_COOKIE['wxd_openid'];
                             $select=M('fusers')->where("openid='$a'")->find();
                              if (empty($select)) {
                                 $data['is_gz']=1;
                                $data['openid']=$a;
                                setcookie('wxd_openid', $a, time() + 31536000);
                                $re=M('fusers')->data($data)->add();
                              }
        ...
        $ismustsub=$this->ismustsub;
        $isonlyweb=$this->isonlyweb;
        $isreduce=$this->isreduce;
        $tourvote=$this->tourvote;
        ...
        if ($IIIIIllI1l1l && empty($_GET['wecha_id'])) {
			...
			//自定义二维数组
			//$IIIIIIIIII1I = M('VOTE')->where(array('id' => $IIIIIllI1l1l))->find();
            ...
            $jiekou = C('site_url') . '/index.php?g=Wap&m=Vote&a=index&token=' . $IIIIIIIIlIlI . '&id=' . $IIIIIllI1l1l . '&page={page}';
            $IIIII11IIlI1 = new PageClass($IIIII11IIlII, $IIIII11IIlIl, $IIIIIIIII11I, $jiekou);
            $IIIII11IIllI = $IIIII11IIlI1->page_limit;
            $IIIIIIIII1ll = $IIIII11IIlI1->myde_size;
        //根据 id 判断生成的排序、
            //修改时间 8.1 针对创客大赛

            if ( $IIIIIIlI1111['isDesc']==1) {
            ...
            $this->assign('url', $url);
            $this->assign('ismustsub',$ismustsub);
             $this->assign('isonlyweb',$isonlyweb);
    ...
    public function rank()
    {
        ...
        if ($IIIIIllI1l1l) {
            ...
            $IIIII11IIlII = count($IIIII11III11);
            $IIIII11IIlIl = $IIIIIIIIII1I['myzps'];
            $jiekou = C('site_url') . '/index.php?g=Wap&m=Vote&a=rank&token=' . $IIIIIIIIlIlI . '&id=' . $IIIIIllI1l1l . '&page={page}';
            $IIIII11IIlI1 = new PageClass($IIIII11IIlII, $IIIII11IIlIl, $IIIIIIIII11I, $jiekou);
            $IIIII11IIllI = $IIIII11IIlI1->page_limit;
            $IIIIIIIII1ll = $IIIII11IIlI1->myde_size;
            $IIIII11IIlll = M('Vote_item')->where($IIIIIllIlII1)->order($IIIII11III1I)->limit($IIIII11IIllI, $IIIIIIIII1ll)->select();
    ...
    public function ticket()
    {
        ...
        if (IS_POST) {
            ...
            $ars= M('vote')->where(array('id'=>$IIIIIllI1l1l))->select();
            $tourvote=$ars[0]['tourvote'];
            $ismustsub=$ars[0]['ismustsub'];
        ...
        if($ismustsub==0){
            ...
            if ($_COOKIE['wxd_openid']) {
                ...
                if ($IIIII11II1lI && $IIIII11II1lI['is_gz'] == 1 || $tourvote== 1) {
                    ...
                    if ($IIIII11II1ll['status'] != 1) {
                        ...
                        if ($IIIIIIlI1111['statdate'] > time()) {
							...
							if($ip_address =='' || is_null($ip_address) ){
								exit('提交的参数非法！');
							}else{
                            $jiekou = 'http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=json&ip=' . $ip_address;
                            $jiekou_bianma = json_decode($this->api_notice_increment($jiekou));
							}
                            if ($IIIIIIIIII1I['xzlx'] > 0 && !empty($IIIIIIIIII1I['area'])) {
                                if ($jiekou_bianma) {
                                    if ($IIIIIIIIII1I['xzlx'] == 1) {
                                        $IIIII11IlI1I = $jiekou_bianma->province;
                                    } elseif ($IIIIIIIIII1I['xzlx'] == 2) {
                                        $IIIII11IlI1I = $jiekou_bianma->city;
                                    }
                                    ...
                                    if ($IIIIIlIllIl1 < $IIIIIIlI1111['ipnubs']) {
                                        if ($IIIIIIllIIll < $IIIIIIlI1111['tpnub']) {
                                            ...
                                            if ($IIIIIIllII1I == 1) {
                                                ...
                                                $IIIIIIllII1l['ip'] = $ip_address;
                                                $IIIIIIllII1l['area'] = $jiekou_bianma->province . $jiekou_bianma->city;
    ...
    public function signup()
    {
        ...
        $url= $websiteurl[0]['url'];
        if (!isset($_SESSION['act'])) {
            header("location:$url/userinfo/index.php?id=$IIIIIllI1l1l");
            return false;
        }
        ...
        if (!isset($IIIIIl1Il1l1) && !isset($IIIII11IIIIl)) {
            if (empty($_COOKIE['wxd_openid'])) {
                if (isset($_GET['wecha_id'])) {
                    ...
                    setcookie('dzp_openid',$IIIIIIlIlIll,time()+31536000,'/','.m.nckyjy.com');
                    */
                    $this->redirect('Wap/Vote/signup', array('id' => $IIIIIllI1l1l, 'token' => $IIIIIIIIlIlI));
                    die;
                }
            } else {
                if (isset($_GET['wecha_id'])) {
            ...
            if ($IIIIIIlI1111) {
                if ($IIIIIIlI1111['start_time'] < time() && $IIIIIIlI1111['over_time'] > time()) {
                    if ($_COOKIE['wxd_openid']) {
                        ...
                        if ($IIIII11II1lI) {
                            //判断活动是否强制关注
                                ($ismustsub==1)?$tsides=1:$tsides=0;
                            if ($IIIII11II1lI['is_gz'] == 1 || $tsides==0) {
  ...
  /*  public function add_vote()
	...
	*/
    ...
    private function savepic($IIIII11I1I11, $IIIIIllI1l1l)
    {
        ...
        $jiekou = 'http://' . $_SERVER['HTTP_HOST'] . $IIIII11I1lIl;
    ...
    private function tcupload($IIIIIlI11II1, $IIIII11I1ll1, $IIIII11I1l1I, $IIIII11I1l1l)
    {
        ...
        $IIIII11I11II = str_replace('s_url:', '', $IIIIII1l1l11[8]);
        if ($IIIII11I11II) {
    ...
    private function GetIP()
    {
        ...
        if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
            ...
            if ($ip_address) {
                array_unshift($IIIII11I11I1, $ip_address);
                $ip_address = FALSE;
            }
            for ($IIIIIIIllI11 = 0; $IIIIIIIllI11 < count($IIIII11I11I1); $IIIIIIIllI11++) {
                if (!eregi('^(10|172\\.16|192\\.168)\\.', $IIIII11I11I1[$IIIIIIIllI11])) {
                    $ip_address = $IIIII11I11I1[$IIIIIIIllI11];
                    break;
                }
            }
        }
        return $ip_address ? $ip_address : $_SERVER['REMOTE_ADDR'];
    }
