...
//首先确定用户是否存在，存在则覆盖
$wonder="SELECT * FROM tp_fusers WHERE openid='$openid' LIMIT 1";
...
$insert="INSERT INTO `tp_fusers` (`nickname`, `sex`, `city`, `country`, `province`, `headimgurl`, `is_gz`,`openid`, `gztime`) VALUES ('$nickname', '$sex', '$city', '$country', '$province', '$headimgurl', '$isgz', '$openid','$gztime')";
    $iiis=$mysql->insert($insert);
}else{
    if($re[0]['nickname']!=$nickname)
    {
        $insert="UPDATE `tp_fusers` SET `nickname`='$nickname',`sex`='$sex',`city`='$city',`country`='$country',`province`='$province',`headimgurl`='$headimgurl',`is_gz`='1',`openid`='$openid',`gztime`='$gztime'  WHERE openid='$openid'";
        $iiis=$mysql->insert($insert);
    }else{
        $iiis=true;
    }
