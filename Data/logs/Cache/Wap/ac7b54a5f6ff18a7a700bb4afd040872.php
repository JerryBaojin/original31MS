...
<div class="text_a clearfix" id="sort">
    ...
    <div id="pageCon" class="match_page masonry" style="padding-bottom: 50px">
        <ul class="list_box masonry clearfix" style="position: relative;">
        			<?php if(is_array($zuopins)): $i = 0; $__LIST__ = $zuopins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i; if($li["group"] == null): ?><li class="picCon">
                        <div>
                            ...
                            <div class="clearfix">
                                ...
                                <?php if($isonlyweb == 0): ?><a href="" class="vote" data-itid="<?php echo ($li["id"]); ?>" data-vote_num="<?php echo ($li["id"]); ?>" data-rule_id="<?php echo ($li["id"]); ?>">投票</a>
                                <?php else: ?>
                                <style type="text/css">
                            ...
                            <li class="picCon">
                                <div>
                                    <i class="number"><?php echo ($li["group"]); ?></i>
                                    <a href="<?php echo U('Vote/detail',array('token'=>$token,'id'=>$id,'zid'=>$li['id']));?>" class="img">
                                    ...
                                    <div class="clearfix">
                                        <span style="float: left;display: block;padding-top: 11px;color:white;"><?php echo ($li["id"]); ?>号</span>
                                        <p>
                                            <?php echo ($li["item"]); ?><br/>
                                            <?php echo ($li["vcount"]); ?>票
                                        </p>
...
$(function($){
	...
	$("#search_form").submit(function(){
		...
		if(keyword.length == 0){alert("<?php echo ($vote['innerText']); ?>");return false;}
	});
});
    $(function(){
                ...
                success: function(data) {
                      ...
                      <?php else: ?>
					   ...
					   $('#voting2').hide();
						...
						$('#voting_content').html('由于该选手拉票频率过高，被系统自动暂时屏蔽，休息一会，再来投票！');
                        $('#voting').show();
					    ...
					    $('#voting2').hide();
						$('#voting_title').html('投票失败');
						$('#voting_content').html('为了平衡后报名的选手，报名期内优先投票最多<?php echo ($vote["btcdxz"]); ?>票！等过了报名期间，就可以正常投票！');
                        $('#voting').show();
