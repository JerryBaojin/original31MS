...
<div class="text_a clearfix" id="sort">
    ...
    <div id="pageCon" class="match_page masonry" style="padding-bottom: 50px">
        <ul class="list_box masonry clearfix" style="position: relative;">
        			<?php if(is_array($zuopins)): $i = 0; $__LIST__ = $zuopins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i; if($li["group"] == '0'): ?><li class="picCon">
                                <div>
...
<div class="bot_main">
    <ul>
        ...
        <?php if($vote["isavote"] == 0): ?><li class="ico_3"><span class="ico"><img src="<?php echo STATICS;?>/vote//index2/i11.png" /></span><span class="txt"><?php if(!empty($ishavezp)): ?>我的<?php else: ?>报名<?php endif; ?></span></li>
            <?php else: ?>
            <style>
                .ico_1,.ico_2,.ico_4{
        ...
        <?php if($user['tpjl']==1 && $user['tpjlnum'] > 0 && $user['gldzpid'] != 0): ?><li class="ico_4"><span class="ico"><img src="<?php echo STATICS;?>/vote/index2/i4.png" /></span><span class="txt">免费抽奖</span></li>
            ...
            <li class="ico_4"><span class="ico"><img src="<?php echo STATICS;?>/vote/index2/i4.png" /></span><span class="txt"><?php echo ($vote["dbdhm"]); ?></span></li><?php endif; ?>
    </ul>
</div>
		...
		$('#voting2').show();
            $.ajax({
                ...
                success: function(data) {
                      if (data == 102) {//未关注
                        window.location.href="http://weixin.scnjnews.com/userinfo/index.php?id=<?php echo ($id); ?>";
    ...
    $(function(){
        $('.vote').on('tap', function(e){
            ...
            $.ajax({
                ...
                success: function(data) {
                  if (data == 102) {//未关注
                       window.location.href="http://v.scnjnews.com/test/index.php?id=<?php echo ($id); ?>";
                    } else if (data == 108) {//投票成功
                         $('#voting2').hide();
						$('#voted').show();
                    } else if (data == 106) {//此用户今日已无法投票
