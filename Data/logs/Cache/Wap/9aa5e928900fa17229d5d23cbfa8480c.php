...
<head>
    ...
    <meta charset="uft-8">
    <?php if($id == 20): ?><title><?php echo ($zpinfo['item']); ?> 编号:<?php echo ($zpinfo['id']); ?>,正在参加<?php echo ($vote['title']); ?>,快来帮我投票吧!</title>
        <?php else: ?>
         <title><?php echo ($zpinfo['item']); ?> 编号:<?php echo ($zpinfo['id']); ?>,正在参加<?php echo ($vote['title']); ?>,快来帮我投票吧!</title><?php endif; ?>
...
<body>
<div id="allgonggao" style="max-width: 640px;width:100%;height:25px;background:rgba(0,0,0,0.6); position:fixed;top:0;z-index:2000; line-height:25px;    max-width: 640px;">
...
if($("#gonggao").text()==""){
$("#allgonggao").css("display","none");
}

if($("#media").attr("src")==""){
$("#audio_btn").css("display","none");
}
...
<section class="content" id="get_info" data-rid="503" data-sort="" data-kw="" data-page="">
    <div class="detial_box">
        ...
        <div class="blank10"></div>
        <?php if($id == 20): ?><video   poster="<?php echo ($zpinfo['startpicurl']); ?>" style="    margin-bottom: 10px;display:block;width: 100%;
    height: auto;" controls="controls"  src="http://otlbcu19c.bkt.clouddn.com/<?php echo ($zpinfo['id']); ?>-1.mp4 "  id="video1" width="100%" >
...
<span>
    <ul id="vvideos">
          ...
          <!--<li  class="options" id="standard" data-href="http://otlbcu19c.bkt.clouddn.com/<?php echo ($zpinfo['id']); ?>-1.mp4 ">标清</li>
             
            <li  class="options" id="high" data-href="http://otlbcu19c.bkt.clouddn.com/<?php echo ($zpinfo['id']); ?>-2.mp4 ">高清</li>-->
           <!-- <li id="surper"  class="options" data-href="http://video.scnjnews.com/ntv/2017/07/24/<?php echo ($zpinfo['id']); ?>-3.mp4 ">超清</li>-->
    </ul>
    <style type="text/css">
    #vvideos{
        display: flex;
...
function chooseColor(){
    console.log('changedColor');
        ...
        var href=video1.src;
        var res=href.slice(href.lastIndexOf("-"));
        switch(res){
            case '-1.mp4':
              $('#standard').addClass('yellow');
              $('#standard').siblings('li').removeClass('yellow');
            break;
             case '-2.mp4':
              $('#high').addClass('yellow');
               $('#high').siblings('li').removeClass('yellow');
            break;
             case '-3.mp4':
              $('#surper').addClass('yellow');
               $('#surper').siblings('li').removeClass('yellow');
            break;
        }
}
   chooseColor();     
        for (var i = 0; i <= 2; i++) {
            $('.options').eq(i).on('tap',function(){
            
                $('#video1').attr('src',$(this).data('href'));
                chooseColor();
            })
        };
    })
            ...
            <?php if(!empty($zpinfo["startpicurl"])): ?><img src="<?php echo ($zpinfo['startpicurl']); ?>" alt=""><?php endif; ?>
					   ...
					   <?php if(!empty($zpinfo["startpicurl5"])): ?><img src="<?php echo ($zpinfo['startpicurl5']); ?>" alt=""><?php endif; endif; ?>
        <?php if(($id != 20)and ($id !=23)): ?><p id="note">参赛宣言：<br /><?php endif; ?>
		<?php  echo html_entity_decode(htmlspecialchars_decode($zpinfo['intro']));?>
		</p>
        <script>
            var ppid=<?php echo ($id); ?>;
    ...
    <div class="abtn_box">
        <?php if($isonlyweb == 0): if(($zpinfo["status"]) == "1"): ?><a href="" class="a_btn toupiao vote" id="vote" data-itid="<?php echo ($zpinfo['id']); ?>">我要投票</a><?php endif; endif; ?>
        <?php if($isonlyweb == 0): if(empty($ishavezp)): ?><a href="<?php echo U('Vote/signup',array('token'=>$token,'id'=>$id));?>" class="a_btn canjia">我也来参加</a><?php endif; endif; ?>
...
<section class="rules">
    <div class="text">
        <?php if(!empty($vote["shumat"])): ?><div class="prize"><?php echo ($vote['shumat']); ?></div>
            <div class="neirong"><?php
 echo html_entity_decode(htmlspecialchars_decode($vote['shuma'])); ?></div><?php endif; ?>
        <?php if(!empty($vote["shumbt"])): ?><div class="prize"><?php echo ($vote['shumbt']); ?></div>
            <div class="neirong"><?php
 echo html_entity_decode(htmlspecialchars_decode($vote['shumb'])); ?></div><?php endif; ?>
    </div>
    <div class="text">
        <?php if(!empty($vote["shumct"])): ?><div class="prize"><?php echo ($vote['shumct']); ?></div>
            <div class="neirong"><?php
 echo html_entity_decode(htmlspecialchars_decode($vote['shumc'])); ?></div><?php endif; ?>
    </div>
...
<section>
    <?php if($ismustsub==0): else: ?>
        <div class="pop" id="guanzhu" style="display:none" >
            <div class="mengceng"></div>
            <div class="pop_up">
                <p class="tit_p"><?php echo ($user["ydgzbt"]); ?></p><br /><!--  引导关注标题 -->
                长按二维码，点击识别关注
                <img id="erweima"  src="<?php echo html_entity_decode(htmlspecialchars_decode($vote['ydgzts'])); ?>" width="180" height="180" style="margin:0 auto;" />
...
<div class="bot_main">
    <ul>
        ...
        <?php if($vote["isavote"] == 0): ?><li class="ico_3"><span class="ico"><img src="<?php echo STATICS;?>/vote//index2/i11.png" /></span><span class="txt"><?php if(!empty($ishavezp)): ?>我的参赛<?php else: ?>报名<?php endif; ?></span></li>
            <?php else: ?>
            <style>
                .ico_1,.ico_2,.ico_4{
  ...
  <style>
        ...
        <!--     <li class="ico_4" style="display:none;"><span class="ico"><img src="<?php echo STATICS;?>/vote/index2/i4.png" /></span><span class="txt"><?php echo ($vote["dbdhm"]); ?></span></li> --><?php endif; ?>
    </ul>
</div>

<script type="text/javascript">

var mp1 ="<?php echo ($vote["mp1"]); ?>" ;
var mp2 ="<?php echo ($vote["mp2"]); ?>" ;
var mp3 ="<?php echo ($vote["mp3"]); ?>" ;
var qr1 ="<?php echo ($vote["qr1"]); ?>" ;
var qr2 ="<?php echo ($vote["qr2"]); ?>" ;
var qr3 ="<?php echo ($vote["qr3"]); ?>" ;
...
wx.onMenuShareTimeline({
           title: "<?php echo ($zpinfo['item']); ?> 编号:<?php echo ($zpinfo['id']); ?>,<?php echo ($vote['sharecontent']); ?>", // 分享标题
            ...
            $.ajax({
                ...
                success: function(data) {
                      if (data == 102) {//未关注
                         ...
                         <?php if($ismustsub==1): ?>$('#voting2').hide();
                      $('#guanzhu').show();
                  <?php else: ?>
                      window.location.href="<?php echo ($url); ?>/userinfo/index.php?id=<?php echo ($id); ?>";<?php endif; ?>
						...
						$('#voting_content').html('由于该选手拉票频率过高，被系统自动屏蔽，请<?php echo ($info['jgfen']); ?>分钟后，再来投票！');
                        $('#voting').show();
                        ...
                        $('#voting').show();
                    } else if(data == 400){//仅能浏览
                      $('#voting2').hide();
                      $('#voting_title').html('无法投票');
                      $('#voting_content').html('请进入公众号，回复选手编号即可进行投票!');
                      $('#voting').show();
...
<?php else: ?>
    $(function(){
        $('.vote').on('tap', function(e){
            e.preventDefault();
            <?php if($id == 20): ?>$('#video1').hide();<?php endif; ?>
            var self = $(e.target).closest('.vote');
			$('#voting2').show();
            $.ajax({
                type: "POST",
                ...
                success: function(data) {
                  if (data == 102) {//未关注
                     console.log(<?php echo ($ismustsub); ?>) ;
                     <?php if($ismustsub==1): ?>$('#voting2').hide();
                      $('#guanzhu').show();
                  <?php else: ?>
                      window.location.href="<?php echo ($url); ?>/userinfo/index.php?id=<?php echo ($id); ?>";<?php endif; ?>
